...
public void reviewPage(){
  if (widgetThres == false){
    ...
    Button PDFExport = new Button(new RectArea(dpw(10),dph(90),dpw(13),dph(10)),DARKRED,LIGHTRED);
    PDFExport.setText("Export PDF",43,0);
    PDFExport.setAction(new Action(){
      public void perform(){
          String monthString = "0"+month();
          monthString = monthString.substring(monthString.length()-2);
          String dayString = "0"+day();
          dayString = dayString.substring(dayString.length()-2);
          String fileName =year()+monthString+dayString;
          pdfReport = (PGraphicsPDF) createGraphics(width, height, PDF, "reports\\"+testerName+"\\"+fileName+".pdf");
          pdfRecord();          
      }
    });
    widgetThres = true;
  ...
  if (keyPressed&&(keyCode==UP||key=='w')){
    ...
    if (heightAdj>displayHeight-dph(35+(mistakes.size()-1)*11+7)){
      heightAdj-=7;
      println("down");
    }
  }
  ...
  fill(0);
  textAlign(NORMAL);
  text("正确答案",dpw(2),dph(30));
  text("题型",dpw(40),dph(30));
  text("你的错误选择",dpw(62),dph(30));
  for (int i=0; i<mistakes.size();i++){
    ...
    textAlign(NORMAL);
    text(displayed.getCorrectAnswer().getWord(),dpw(2),dph(35+i*11));
    text(displayedCorrectDefi,dpw(2),dph(35+i*11+5.5));
    text(displayed.getQuestionType(),dpw(40),dph(35+i*11));
    text(displayedYourDefi,dpw(62),dph(35+i*11));
  }
  translate(0,-heightAdj);
  widgets.display();
